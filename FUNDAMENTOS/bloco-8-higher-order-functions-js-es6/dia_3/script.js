...
const bookDescription = books.map((book) => {
  const bookName = book.name.toUpperCase().split(' ').join('_');
  const bookGenre = book.genre.split(' ').join('_').toUpperCase();
  const bookAuthor = book.author.name.split(' ').join('_').toUpperCase();
